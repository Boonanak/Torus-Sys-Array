...
module memory_sync_tb();
    ...
    localparam W_I_VEC_WIDTH  = ELEMENTS * W_I_WIDTH;  
    localparam PSUM_VEC_WIDTH = ELEMENTS * PSUM_WIDTH; 
    ...
    initial begin
        ...
        // ---------------------------------------------------------
        // 1. LOCAL PORT ACCESSES (Synchronous Read: Data appears cycle N+1)
        // ---------------------------------------------------------
        $display("--- TEST 1: Synchronous Local Port Accesses ---");
        
        // Weight Bank
        weight_w_v = 1; weight_w_addr = 'd10; weight_w_data = 64'h1111_2222_3333_4444;
        @(posedge clk); weight_w_v = 0;
        
        weight_r_v = 1; weight_r_addr = 'd10; 
        @(posedge clk); // Read requested at posedge
        weight_r_v = 0; // De-assert request
        @(posedge clk); // Data valid now
        if (weight_r_data === 64'h1111_2222_3333_4444) $display("[PASS] Local Weight Sync");
        else $display("[FAIL] Local Weight Sync. Got %h", weight_r_data);

        // Input Bank
        input_w_v = 1; input_w_addr = 'd20; input_w_data = 64'h5555_6666_7777_8888;
        @(posedge clk); input_w_v = 0;
        
        input_r_v = 1; input_r_addr = 'd20; 
        @(posedge clk);
        input_r_v = 0;
        @(posedge clk);
        if (input_r_data === 64'h5555_6666_7777_8888) $display("[PASS] Local Input Sync");
        else $display("[FAIL] Local Input Sync. Got %h", input_r_data);

        // Psum Bank
        psum_w_v = 1; psum_w_addr = 'd30; psum_w_data = 128'hAAAA_BBBB_CCCC_DDDD_EEEE_FFFF_0000_1111;
        @(posedge clk); psum_w_v = 0;
        
        psum_r_v = 1; psum_r_addr = 'd30; 
        @(posedge clk);
        psum_r_v = 0;
        @(posedge clk);
        if (psum_r_data === 128'hAAAA_BBBB_CCCC_DDDD_EEEE_FFFF_0000_1111) $display("[PASS] Local Psum Sync");
        else $display("[FAIL] Local Psum Sync. Got %h", psum_r_data);

        // ---------------------------------------------------------
        // 2. CROSS PORT ACCESSES (Synchronous Read)
        // ---------------------------------------------------------
        $display("\n--- TEST 2: Synchronous Cross Port Accesses ---");

        // 2a. Local Weight Write -> Chip IO Read (Zero Padding Check)
        ...
        if (chip_io_r_data === {64'h0, 64'hDEAD_BEEF_CAFE_BABE}) 
            $display("[PASS] Cross Weight Read Sync (Correct Padding)");
        else 
            $display("[FAIL] Cross Weight Read Sync. Got %h", chip_io_r_data);
        ...
        psum_r_v = 1; psum_r_addr = 'd15; 
        @(posedge clk);
        psum_r_v = 0;
        @(posedge clk);
        if (psum_r_data === 128'h1234_5678_90AB_CDEF_1234_5678_90AB_CDEF)
            $display("[PASS] Cross Psum Read Sync");
        else
            $display("[FAIL] Cross Psum Read Sync. Got %h", psum_r_data);
        ...
        // Address 0 (Weight via Chip IO)
        chip_io_w_v = 1; chip_io_w_addr = {TAG_W'(0), ADDR_W'(0)};
        chip_io_w_data = 128'h1; @(posedge clk); chip_io_w_v = 0;
        
        weight_r_v = 1; weight_r_addr = 0; 
        @(posedge clk);
        weight_r_v = 0;
        @(posedge clk);
        if (weight_r_data === 64'h1) $display("[PASS] Boundary Addr 0 Sync");
        else $display("[FAIL] Boundary Addr 0 Sync. Got %h", weight_r_data);

        // Max Address (Psum via Local Port)
        ...
        chip_io_r_v = 0;
        @(posedge clk);
        if (chip_io_r_data === {128{1'b1}}) $display("[PASS] Boundary Addr Max (%0d) Sync", DEPTH-1);
        else $display("[FAIL] Boundary Addr Max Sync. Got %h", chip_io_r_data);

        $display("\nSynchronous memory verification complete.");
        $finish;
    end
