...
trace_lines.append(generate_32b_bin_flit(0)) # [cite: 35]
...
trace_lines.append(generate_32b_bin_flit(16, addr=1)) # [cite: 35]
trace_lines.append("# Data Low")
trace_lines.append(to_bin(0x55667788, 32))
trace_lines.append("# Data High")
trace_lines.append(to_bin(0x11223344, 32))

trace_lines.append("# READV8 Addr=0x01")
trace_lines.append(generate_32b_bin_flit(8, addr=1)) # [cite: 37]
...
trace_lines.append(generate_32b_bin_flit(16, addr=128))
for i in range(8):
    trace_lines.append(f"# Partial Sum Flit {i}")
    trace_lines.append(to_bin(0xAAAA0000 + i, 32))

# --- 4. Matrix Operations ---
# Load weights from Matrix 61 (Identity Matrix) 
trace_lines.append("# LR BaseAddr=61 (Load Identity Matrix weights)")
trace_lines.append(generate_32b_bin_flit(56, baddr_weight=61)) # [cite: 37]

# Compute A * B + C [cite: 38]
trace_lines.append("# CR Dest=2, Src=0, Acc=62 (Compute using hard-coded Zero Matrix)")
trace_lines.append(generate_32b_bin_flit(38, baddr_dest=2, baddr_src=0, baddr_acc=62)) # [cite: 38, 49]

# Simultaneous Load and Compute [cite: 39]
trace_lines.append("# LCCR Dest=3, Src=0, Weight=5, Acc=2")
trace_lines.append(generate_32b_bin_flit(54, baddr_dest=3, baddr_src=0, baddr_weight=5, baddr_acc=2)) # [cite: 39]

# Transpose Matrix [cite: 35]
trace_lines.append("# TRANSPOSE Matrix 3 to Matrix 4")
trace_lines.append(generate_32b_bin_flit(25, baddr_dest=4, baddr_src=3)) # [cite: 35]

# --- 5. Bulk Matrix Reads ---
trace_lines.append("# READM8 BaseAddr=4")
trace_lines.append(generate_32b_bin_flit(9, baddr_src=4)) # [cite: 37]

# Reading Matrix 62 (Hard-coded Zero Matrix) 
trace_lines.append("# READM16 BaseAddr=62")
trace_lines.append(generate_32b_bin_flit(11, baddr_src=62)) # [cite: 37]

# --- 6. Status and CSR ---
trace_lines.append("# ERROR CSR (Clear status flags)")
trace_lines.append(generate_32b_bin_flit(28)) # [cite: 35]
